...
NTL.phys.data1999 <- NTL.phys.data %>% filter( year4 == 1999)


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.dataTues <- NTL.phys.data %>% 
  filter( lakename == "Tuesday Lake", year4 %in% c(1990:1999))


unique(NTL.phys.dataTues$year4)
...
ex3 %>% 
  filter( depth == 0) %>% 
  summarise(mean_surface_tempC =  mean( na.omit( temperature_C)))
...
ex5 <- 
  NTL.phys.data %>% 
  pivot_longer( cols = c(irradianceWater, irradianceDeck),
                names_to = "irradiance", 
                values_to = "rads")
