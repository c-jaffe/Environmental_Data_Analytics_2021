...
sum(is.na(BRA_complete$Rainfall))
sum(is.na(BRA_complete$Temperature_C))
...
f_year <- month(first(BRA_complete$Date))

BRA_Temp_ts <- ts(BRA_complete$Temperature_C, 
                  start = c(f_year,f_month), 
                  frequency = 12)

BRA_Rainfall_ts <- ts(BRA_complete$Rainfall, 
                      start = c(f_year,f_month), 
                      frequency = 12)
```
...
Temp_decomp <- 
stl(BRA_Temp_ts, s.window = "periodic")
plot(Temp_decomp)

Rainfall_decomp <- 
stl(BRA_Rainfall_ts, s.window = "periodic")
...
Temp_Components <- as.data.frame(Temp_decomp$time.series)

Temp_Components_df <- mutate(Temp_Components,
...
Temp_plot <- 
  ggplot(Temp_Components_df) +
  geom_line(aes(y = Observed_Temp, x = Date),  size = 0.25, color = "grey") +
...
Temp_trend <- 
  Kendall::SeasonalMannKendall(BRA_Temp_ts)
Temp_trend2 <- 
  smk.test(BRA_Temp_ts)
...
ts_temp_noSeason <- ts(data = Temp_Components_df$Observed_Temp - Temp_Components_df$seasonal, 
                       start = c(f_year,f_month), 
                       frequency = 12)

# rerun mk
temp_mk <- Kendall::MannKendall(ts_temp_noSeason)
summary(temp_mk)


# before plotting add no season observed values to component dataframe

Temp_Components_df_NS <- Temp_Components_df %>%
  mutate( obv_noseason = Observed_Temp - seasonal)

# plot
### Q: which data to se to plot lines? double check against video
Temp_plot_NS <- 
  ggplot(Temp_Components_df_NS) +
  geom_line(aes(y = obv_noseason, x = Date),  size = 0.25, color = "grey") +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") 
