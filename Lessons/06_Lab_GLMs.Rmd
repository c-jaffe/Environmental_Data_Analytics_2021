...
> Answer: Our null hypo is that ozone levels are not below the threshold for good AQI, that is, they are greater than 50. alternative is that they are =< 50.
...
> On average ozone measured in NC is less than 50ppm ( t-test, df = 6829, p < .001)
...
EPAairSubset <- EPAair %>% select(Month, Year, lat_mean:Ozone) %>%  na.omit()

EPAairCor <- cor(EPAairSubset)
...
lm <- lm(data = EPAair, PM2.5 ~ Ozone + Month + Year + lat_mean + lon_mean)
summary(lm)
