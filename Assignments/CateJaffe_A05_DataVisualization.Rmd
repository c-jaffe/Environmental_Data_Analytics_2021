...
lakesTidy <- read.csv("./Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)

lakesGathered <- read.csv("./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv", stringsAsFactors = TRUE)

neonLitter <- read.csv("./Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv", stringsAsFactors = TRUE)
...
lakesTidy %>% 
  ...
  scale_color_manual( values = c("#0b61a1ff","#ed7933ff" )) + # set "lakename" variable (two levels) to two colors from "portland" colormap
  geom_smooth( method = "lm", color = "black", se = FALSE) + # line with linear fit, no display of standard error
  ylab("Total Phosphorus (ug)") + 
  xlab("Phosphate (ug)") + 
  xlim(0,50) # adjust x axis to exclude extreme phosphate value
...
# temperature
plotTemp <- 
ggplot(lakesTidy, aes( x = as.factor(month) , y = temperature_C, color = lakename)) + # month as.factor() so R displays x axis values as discrete
  geom_boxplot() + 
  scale_color_manual( values = c("#0b61a1ff", "#ed7933ff" ) ) + # match colors to previous scatter plot
  xlab("") + 
  ylab("Temperature (C)")

# total P
plotTP <- 
ggplot(lakesTidy, aes( x = as.factor(month) , y = tp_ug, color = lakename)) + 
  geom_boxplot() + 
...
# total N
plotTN <- 
ggplot(lakesTidy, aes( x = as.factor(month) , y = tn_ug, color = lakename)) +
  geom_boxplot() + 
  scale_color_manual( values = c("#0b61a1ff", "#ed7933ff" ) ) + 
  xlab("Month") + 
...
neonLitter %>% filter(functionalGroup == "Needles") %>% 
  ...
  scale_x_date( breaks = "6 months",
                date_labels = "%b %Y",
                limits = c(as.Date("2016-05-01"), max(neonLitter$collectDate))) + # shift limits so date 6-mo labels are closer to sampling dates
  theme(axis.text.x = element_text( angle = 45, 
                                    hjust = 1)) + # tilt date on x axis by 45 degrees
