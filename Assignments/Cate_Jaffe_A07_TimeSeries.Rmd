...
GaringerOzone.monthly <- 
  GaringerOzone %>% group_by(month(Date), year(Date)) %>% 
  summarise( mean = mean(ozone_clean)) %>% 
  # rename month and year columns in grouped df
  rename("month" = "month(Date)", "year" = "year(Date)" ) %>% 
  # new Date column with month, year combo
  # set day to 01 for all dates and convert to date format
  mutate(month_year = mdy(paste(month,"-01-",year)))
