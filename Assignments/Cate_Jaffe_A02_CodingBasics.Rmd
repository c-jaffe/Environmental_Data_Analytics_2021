...
#2. Use functions mean() and median() to generate numeric list of the mean and median of each number in the sequence created above

mean(seqBy4)

median(seqBy4)

#3. Check if the mean of the sequence is greater than the median using logical statement
...
scores <- c(83, 49, 98, 90) # numeric vector, score listed in order to match names
PF <- c(TRUE, FALSE, TRUE, TRUE) # logical vector, TRUE = "pass" FALSE = "fail", passing grade is 50
...
PassFailCheck <- function(grade) { # the only input to the function will be the test score
  
  results <- ifelse(grade > 50, "PASS", "FAIL") # if the score is greater than 50, the student passes, otherwise the student fails.
 
   print(results) # print results of "PASS" or "FAIL"
}
